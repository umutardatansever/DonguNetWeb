    ...
    <div className={styles.container}>
      ...
      <div className={styles.statsGrid}>
        <div className="glass-panel p-4 md:p-6 rounded-2xl flex flex-col justify-between">
          ...
          <span className="text-xl md:text-3xl font-extrabold font-title text-on-surface mt-2 block">68%</span>
        ...
        <div className="glass-panel p-4 md:p-6 rounded-2xl flex flex-col justify-between">
          ...
          <span className="text-xl md:text-3xl font-extrabold font-title text-on-surface mt-2 block">14 Tesis</span>
        </div>
        <div className="glass-panel p-4 md:p-6 rounded-2xl flex flex-col justify-between">
          <span className="text-[10px] md:text-xs font-semibold text-on-surface-variant uppercase tracking-wider block">
            Toplam CO2
          </span>
          <span className="text-xl md:text-3xl font-extrabold font-title text-teal-700 mt-2 block">2.1 Ton</span>
        </div>
        <div className="glass-panel p-4 md:p-6 rounded-2xl flex flex-col justify-between">
          <span className="text-[10px] md:text-xs font-semibold text-on-surface-variant uppercase tracking-wider block">
            Karbon Tasarrufu
            ...
            <svg viewBox="0 0 600 320" className="w-full h-full text-on-surface-variant" fill="currentColor">
              <defs>
                ...
                <radialGradient id="heat-high" cx="50%" cy="50%" r="50%">
                  <stop offset="0%" stopColor="rgba(30, 95, 70, 0.3)" />
                  <stop offset="100%" stopColor="rgba(30, 95, 70, 0)" />
                </radialGradient>
                <radialGradient id="heat-mid" cx="50%" cy="50%" r="50%">
                  <stop offset="0%" stopColor="rgba(15, 122, 108, 0.2)" />
                  <stop offset="100%" stopColor="rgba(15, 122, 108, 0)" />
                </radialGradient>
              </defs>
              ...
              <polygon
                ...
                fill="none"
                stroke="rgba(30, 95, 70, 0.25)"
                strokeWidth={2}
                strokeDasharray="8 4"
              />
              <text x="70" y="55" fill="rgba(30, 95, 70, 0.5)" fontSize="10" fontWeight="bold">
                GEBZE ORGANIZE SANAYI BOLGESI SINIRI
              </text>
              <path d="M 120 120 Q 250 80 380 140" fill="none" stroke="#1e5f46" strokeWidth={2} strokeOpacity={0.6} />
              <path d="M 380 140 Q 300 220 220 200" fill="none" stroke="#0f7a6c" strokeWidth={1.5} strokeOpacity={0.4} />
              <circle r="4" fill="#2f8261">
                <animateMotion dur="6s" repeatCount="indefinite" path="M 120 120 Q 250 80 380 140" />
              </circle>
              <circle cx={120} cy={120} r={8} fill="#1e5f46" />
              <text x="135" y="123" fill="#666f63" fontSize="9" fontWeight="bold">
                Gebze Metal A.Ş.
              </text>
              <circle cx={380} cy={140} r={8} fill="#1e5f46" />
              <text x="395" y="143" fill="#666f63" fontSize="9" fontWeight="bold">
                Dilovası Doküm
              </text>
              <circle cx={220} cy={200} r={8} fill="#0f7a6c" />
              <text x="235" y="203" fill="#666f63" fontSize="9" fontWeight="bold">
                Kocaeli Cam
              </text>
              <circle cx={450} cy={220} r={6} fill="#a8a290" />
              <text
                x={465}
                y={223}
                fill={osbVerificationList[1].status === "approved" ? "#666f63" : "#8b8676"}
                fontSize="9"
                fontWeight={osbVerificationList[1].status === "approved" ? "bold" : "normal"}
              >
                Marmara Kağıt
              </text>
              {osbVerificationList[1].status === "approved" && <circle cx={450} cy={220} r={6} fill="#1e5f46" />}
            </svg>
          </div>
        </div>
